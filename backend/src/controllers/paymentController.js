...
const createPayment = async (req, res, next) => {
  try {
    ...
    const result = await query(
      `INSERT INTO payments (booking_id, amount, method, transaction_id, status, provider, phone_number, paid_at, metadata)
       VALUES ($1, $2, $3, $4, $5, 'rindaseat_demo', $6, NOW(), $7::JSONB)
       RETURNING id, booking_id, amount, method, transaction_id, status, provider, phone_number, paid_at, created_at`,
      [
...
const verifyPayment = async (req, res, next) => {
  try {
    ...
    const paymentResult = await query(
      `SELECT id, booking_id, amount, method, transaction_id, status, paid_at, created_at
       FROM payments
...
const getPaymentStatus = async (req, res, next) => {
  try {
    const result = await query(
      `SELECT payments.id, payments.booking_id, payments.amount, payments.method, payments.transaction_id, payments.status, payments.paid_at, payments.created_at
       FROM payments
